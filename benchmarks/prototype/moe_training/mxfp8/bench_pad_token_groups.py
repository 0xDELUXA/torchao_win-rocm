...
class ExperimentResult:
    ...
    triton_fused_time_us: float
    cuda_time_us: float
    torch_mem_bw_gbps: float
    triton_mem_bw_gbps: float
    triton_fused_mem_bw_gbps: float
...
triton_pad_token_groups_c = torch.compile(
    triton_pad_token_groups, fullgraph=True, dynamic=False
)

triton_fused_pad_token_groups_c = torch.compile(
    triton_fused_pad_token_groups, fullgraph=True, dynamic=False
)
...
def run_experiment(
    ...
    def triton_with_offsets():
        ...
        return triton_pad_token_groups_c(inputs, group_offsets, alignment_size)

    def triton_fused_with_offsets():
        group_offsets = generate_jagged_offs(
            num_groups, num_tokens, multiple_of=1, device=device
        )
        return triton_fused_pad_token_groups_c(inputs, group_offsets, alignment_size)
    ...
    # bench triton fused (use host-side timing for consistency)
    warmup(triton_fused_with_offsets)
    triton_fused_time_us = benchmark_host_side_in_microseconds(
        triton_fused_with_offsets
    )
    if args.profile:
        group_offsets = generate_jagged_offs(
            num_groups, num_tokens, multiple_of=1, device=device
        ...
        profile_fn(
            ...
            group_offsets,
            alignment_size,
            profile_name="triton_fused_pad_token_groups",
        )
    ...
    triton_fused_mem_bw_gbps = (total_bytes / 1e9) / (triton_fused_time_us / 1e6)
    ...
    return ExperimentResult(
        ...
        triton_fused_time_us=triton_fused_time_us,
        cuda_time_us=cuda_time_us,
        torch_mem_bw_gbps=torch_mem_bw_gbps,
        triton_mem_bw_gbps=triton_mem_bw_gbps,
        triton_fused_mem_bw_gbps=triton_fused_mem_bw_gbps,
...
def print_results(experiments: List[Experiment]):
    headers = [
        ...
        "torch_mem_bw_gbps",
        "triton_mem_bw_gbps",
        "triton_fused_mem_bw_gbps",
        "cuda_mem_bw_gbps",
        "triton_vs_torch",
        "fused_triton_vs_torch",
        "cuda_vs_torch",
    ]
    rows = []
    for experiment in experiments:
        ...
        rows.append(
            [
                ...
                experiment.result.triton_fused_time_us,
                experiment.result.cuda_time_us,
                f"{experiment.result.torch_mem_bw_gbps:.2f}",
                f"{experiment.result.triton_mem_bw_gbps:.2f}",
                f"{experiment.result.triton_fused_mem_bw_gbps:.2f}",
                cuda_bw_str,
                f"{experiment.result.torch_eager_time_us / experiment.result.triton_time_us:.2f}x",
                f"{experiment.result.torch_eager_time_us / experiment.result.triton_fused_time_us:.2f}x",
                cuda_vs_torch,
            ]
        )
